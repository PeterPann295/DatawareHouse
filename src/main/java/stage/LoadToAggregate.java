    ...
    public static void main(String[] args) throws ParseException, SQLException, InterruptedException {
        ...
        try (Connection connection = db.getConnection()) {
            ...
            if (fileLog == null) {
                ...
                dao.insertLog(connection, fileLog.getId(), "FAILED", "LoadToArrgerate thất bại");
            }
            // 6. Kiểm tra status log_file phải WH_LOADED?
            if (fileLog.getStatus().equalsIgnoreCase("WH_LOADED")) {
                controller.loadToAggregate(connection, fileLog);
            } else {
                // 5.1 Ghi log LoadToAggregate dữ liệu file_log thất bại
                dao.insertLog(connection, fileLog.getId(), "FAILED", "LoadToAggregate thất bại");
            }
        }
    }
}
